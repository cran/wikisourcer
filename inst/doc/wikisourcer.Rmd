...
The **wikisourcer** R package helps you download any book or page from Wikisource. The text is downloaded in a tidy data frame, so it can be analyzed within the tidyverse ecosystem as explained for example in the book *[Text mining with R](https://www.tidytextmining.com)*.
...
A [tidy sentiment analysis](https://www.tidytextmining.com/sentiment.html) of a book by chapter can easily be made using the ```wikisource_book``` function, as the chapters are automatically created in the ```page``` variable.
